...
We implemented linear (for quantitative outcome) and logistic (for binary outcome) model in Mendelian randomization. Note that for hypothesis testing purposes, a linear model can also be used for dichotomous outcomes. In general, linear models are more efficiency.
